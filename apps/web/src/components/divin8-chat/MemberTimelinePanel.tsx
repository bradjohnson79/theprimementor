...
import { classNames, type Divin8TimelineEvent } from "@wisdom/ui/divin8-chat";
...
function typeLabel(type: Divin8TimelineEvent["type"]) {
  switch (type) {
...
export default function MemberTimelinePanel({ events, isLightTheme = false }: MemberTimelinePanelProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-sm font-semibold text-accent-cyan">Insight Timeline</p>
        <p className={classNames("mt-1 text-xs leading-5", isLightTheme ? "text-slate-500" : "text-white/55")}>
          Live continuity for the active thread. Confirmed facts and recent engine moments stay visible here.
        </p>
      </div>

      {safeEvents.length === 0 ? (
        <p className={classNames("text-sm", isLightTheme ? "text-slate-500" : "text-white/55")}>
          Timeline events will appear here as the conversation builds memory.
        </p>
      ) : (
        <div className="space-y-2">
          {safeEvents.map((event) => {
            ...
            const summary =
              event.summary.length > 120 && !isExpanded
